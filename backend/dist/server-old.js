...
async function sendPaymentConfirmedEmail(to, winnerName, auctionTitle) {
    try {
        await resend.emails.send({
            from: process.env.RESEND_FROM_EMAIL || 'onboarding@resend.dev',
            to,
...
async function markWinnerPaidRazorpay(winnerId, paymentId) {
    ...
    if (bidder?.email) {
        await sendPaymentConfirmedEmail(bidder.email, bidder?.name || 'Winner', auction?.title || 'Auction');
    }
    return { ok: true };
}
...
api.get('/auction/active', async (_req, res) => {
    try {
        await finalizeEndedAuctions();
        const now = new Date().toISOString();
        const { data: auctions, error } = await supabaseAdmin
...
api.get('/auction/product/:product_id', async (req, res) => {
    try {
        await finalizeEndedAuctions();
...
api.get('/auction/:id', async (req, res) => {
    try {
        ...
        if (data.status === 'live' && hasSizes) {
            ...
            data.highest_bids_by_size = sizes.map((s) => ({ size: s, amount: bySize[s]?.amount ?? 0, bidder_name: bySize[s]?.bidder_name ?? null }));
        }
        setNoCache(res);
...
api.get('/auction-widget', async (_req, res) => {
    const widgetScript = `
(function() {
  'use strict';
...
api.post('/auth/check-user', async (req, res) => {
    try {
...
api.post('/auth/send-email-otp', async (req, res) => {
    try {
        const body = req.body || {};
...
api.post('/auth/verify-email-otp', async (req, res) => {
    try {
        const body = req.body || {};
        const { code, name, email, phone } = body;
        if (!code || !email) {
...
api.post('/auth/verify-test-otp', async (_req, res) => {
...
api.post('/register-bidder', async (req, res) => {
    try {
        const body = req.body || {};
...
// Public: Place bid
api.post('/place-bid', async (req, res) => {
    try {
        const body = req.body || {};
...
api.get('/shopify-drops', async (_req, res) => {
    try {
...
api.post('/cron/check-winner-payments', async (req, res) => {
...
api.get('/winner/claim', async (req, res) => {
    try {
...
api.post('/winner/create-order', async (req, res) => {
    try {
        if (!razorpay) {
        ...
        if (!orderId) {
            ...
            const order = await new Promise((resolve, reject) => {
                razorpay.orders.create({ amount: amountPaise, currency: 'INR', receipt }, (err, order) => (err ? reject(err) : resolve(order)));
            });
            orderId = order.id;
...
api.post('/winner/verify-payment', async (req, res) => {
    try {
        ...
        if (razorpay) {
            const payment = await new Promise((resolve, reject) => {
                ;
                razorpay.payments.fetch(razorpay_payment_id, (err, p) => (err ? reject(err) : resolve(p)));
            });
...
api.post('/winner/webhook', async (req, res) => {
    try {
        ...
        const expectedSig = crypto_1.default.createHmac('sha256', razorpayWebhookSecret).update(rawBody).digest('hex');
...
admin.get('/auctions', async (_req, res) => {
    try {
        const { data: auctions, error } = await supabaseAdmin
            .from('auctions')
            .select('*')
...
admin.post('/auctions', maybeUpload, async (req, res) => {
    try {
        const contentType = req.headers['content-type'] || '';
...
admin.get('/auctions/:id', async (req, res) => {
    try {
...
admin.put('/auctions/:id', async (req, res) => {
    try {
        const auctionId = req.params.id;
        const body = req.body || {};
        ...
        if (status === 'ended') {
            const sizes = Array.isArray(updatedAuction?.available_sizes) ? updatedAuction.available_sizes : [];
...
admin.delete('/auctions/:id', async (req, res) => {
    try {
        const auctionId = req.params.id;
...
admin.get('/bidders', async (_req, res) => {
    try {
        const { data: bidders, error: biddersError } = await supabaseAdmin
            .from('bidders')
            .select(`
...
admin.get('/shopify-drops', async (req, res) => {
    try {
...
admin.post('/shopify-drops', async (req, res) => {
    try {
        const body = req.body || {};
...
admin.patch('/shopify-drops', async (req, res) => {
    try {
        const body = req.body || {};
...
admin.delete('/shopify-drops', async (req, res) => {
    try {
...
admin.post('/upload-url', async (req, res) => {
    try {
...
admin.get('/winners', async (_req, res) => {
    try {
...
admin.patch('/winners/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body || {};
