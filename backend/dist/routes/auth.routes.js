...
router.post('/auth/send-otp', async (req, res) => {
    try {
        ...
        if (!services_1.twilioClient || !env_1.env.twilioAccountSid || !env_1.env.twilioAuthToken) {
        ...
        try {
            await services_1.twilioClient.verify.v2
...
router.post('/auth/verify-otp', async (req, res) => {
    try {
        ...
        const verification = await services_1.twilioClient.verify.v2
            .services(verifyServiceSid)
...
router.post('/auth/send-email-otp', async (req, res) => {
    try {
        ...
        const { data: recentOtps } = await supabase_1.supabaseAdmin
            .from('email_otps')
            .select('id')
            .eq('email', normalizedEmail)
        ...
        if (services_1.resend) {
            try {
                await services_1.resend.emails.send({
                    from: env_1.env.resendFromEmail || 'onboarding@resend.dev',
                    to: normalizedEmail,
...
router.post('/auth/verify-email-otp', async (req, res) => {
    try {
        ...
        const normalizedEmail = email.toLowerCase().trim();
        const { data: otpRecord, error: otpError } = await supabase_1.supabaseAdmin
            .from('email_otps')
            .select('*')
            .eq('email', normalizedEmail)
            .eq('otp_code', code)
...
router.post('/auth/verify-test-otp', async (_req, res) => {
