...
router.get('/auction/:id', async (req, res) => {
    try {
        ...
        const { data: highestBid } = await supabase_1.supabaseAdmin
            ...
            .maybeSingle();
        // Get total bids
        const { count: totalBids } = await supabase_1.supabaseAdmin
            .from('bids')
            .select('id', { count: 'exact', head: true })
            .eq('auction_id', id);
        ...
        const sizes = Array.isArray(auction.available_sizes)
            ...
            : [];
        let highest_bids_by_size = null;
        let locked_bid_size = null;
        let size_lock_active = false;
        if (sizes.length > 0) {
            const { data: firstBid } = await supabase_1.supabaseAdmin
                .from('bids')
                .select('size')
                .eq('auction_id', id)
                ...
                .limit(1)
                .maybeSingle();
            const firstBidSize = String(firstBid?.size ?? '').trim();
        ...
        if (sizes.length > 0) {
            const { data: bids } = await supabase_1.supabaseAdmin
                .from('bids')
