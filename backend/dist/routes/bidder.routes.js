...
router.post('/register-bidder', async (req, res) => {
    try {
        ...
        const { data: newBidder, error: insertError } = await supabase_1.supabaseAdmin
            .from('bidders')
            .insert(bidderData)
            .select('id')
            .single();
...
router.post('/place-bid', async (req, res) => {
    try {
        ...
        const configuredSizes = Array.isArray(auction.available_sizes)
            ? auction.available_sizes.map((s) => String(s ?? '').trim()).filter((s) => s.length > 0)
            : [];
        if (configuredSizes.length > 0) {
            ...
            const { data: firstBidForBidder } = await supabase_1.supabaseAdmin
                .from('bids')
                .select('size')
                .eq('auction_id', auction_id)
                ...
                .limit(1)
                .maybeSingle();
            const lockedBidSize = String(firstBidForBidder?.size ?? '').trim() || null;
        ...
        let highestBidQuery = supabase_1.supabaseAdmin
            .from('bids')
            .select('amount')
            .eq('auction_id', auction_id)
        ...
        // 5. Insert bid
        const { data: newBid, error: bidError } = await supabase_1.supabaseAdmin
            .from('bids')
            .insert({
            auction_id,
            bidder_id,
...
router.get('/bidder-size-lock', async (req, res) => {
    try {
        ...
        const { data: firstBid, error: firstBidError } = await supabase_1.supabaseAdmin
            .from('bids')
            .select('size')
            .eq('auction_id', auction_id)
            .eq('bidder_id', bidder_id)
        ...
        const locked_size = String(firstBid?.size ?? '').trim() || null;
        return res.json({
            success: true,
            auction_id,
            bidder_id,
