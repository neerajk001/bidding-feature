...
let lastWinnerEmailError = null;
const DEFAULT_FROM = 'onboarding@resend.dev';
function getLastWinnerEmailError() {
    return lastWinnerEmailError;
}
function setLastWinnerEmailError(reason) {
...
function getFromEmail() {
    return (env_1.env.resendFromEmail || DEFAULT_FROM).trim();
}
function getReplyToEmail() {
    const replyTo = (env_1.env.resendReplyToEmail || '').trim();
    return replyTo || undefined;
}
function isResendSandboxSender(fromEmail) {
    ...
    try {
      ...
      `
        ...
        if (error) {
            ...
            const reason = error?.message || error?.name || JSON.stringify(error);
