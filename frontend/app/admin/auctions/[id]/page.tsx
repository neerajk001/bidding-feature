  ...
  const fetchAuctionData = async () => {
    try {
      ...
      if (ok && data.auction) {
        ...
        setBids(data.bids ?? [])
        setCurrentHighestBid(data.current_highest_bid ?? null)
        setWinnersBySize(data.winners_by_size ?? [])
            ...
            if (auction.status === 'ended' && hasSizes && winnersBySize.length > 0) {
              return (
                <>
                  ...
                  <div className="text-gray-600 mt-4">{bids.length} total bids</div>
                </>
              )
            }
            if (auction.status === 'ended' && !hasSizes && winnersBySize.length > 0) {
              ...
              return (
                <>
                  ...
                  <div className="text-gray-600">{bids.length} total bids</div>
                </>
              )
            }
